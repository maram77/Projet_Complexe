...
export class CartService {
  ...
  getAllProducts(): Observable<Product[]> {
    return this.http.get<Product[]>(`http://localhost:8080/api/auth/products/all`);
  }

  checkout(userId: number, cartId: number, delivery: any): Observable<any> {
    const url = `http://localhost:8080/api/auth/checkout/process?userId=${userId}&cartId=${cartId}`;
