...
import com.example.web_project.Dto.OrderStatus;
...
public class CheckoutController {
    ...
    public String sendOrderStatus(@RequestBody OrderStatus orderStatus) {
        try {
             orderDetailService.SendUpdateOrder(orderStatus.getOrder() ,orderStatus.getStatus());
            return "Order status sent!";
        } catch (Exception e) {
            e.printStackTrace();
            return "Failed to send order status";
        }
    }
